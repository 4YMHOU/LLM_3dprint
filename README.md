...
Целью работы являлось дообучение большой языковой модели **Qwen2.5-1.5B** на узкоспециализированной тематике — **3D-печать**.  
...
Использовался **Parameter-Efficient Fine-Tuning (PEFT)** подход, в частности **LoRA (Low-Rank Adaptation)**, который позволяет дообучать модель, добавляя всего **0.05% новых параметров** (примерно 8 милилонов. из 1.5 млрд).
...
Ниже представлен скриншот процесса обучения модели в терминале, показывающий динамику потерь (loss) на каждом шаге обучения:

![Скриншот процесса обучения](Обучение.jpg)
*Рис. 1: Процесс обучения модели в терминале*

### График потерь (Loss)

После обучения был сохранён график динамики функции потерь, который показывает эффективность процесса обучения:

![Динамика обучения](training_loss.png)
*Рис. 2: Динамика функции потерь во время обучения модели*

### Анализ графика потерь
- **Начальное значение loss**: 1.9284 (шаг 0)
- **Финальное значение loss**: 1.0823 (шаг 60)
- **Общее снижение loss**: ~44% за 3 эпохи
- **Стабильность обучения**: Постепенное снижение без резких скачков
- **Количество шагов**: 60 (20 шагов на эпоху)
...
- **Время обучения**: 37 минут 55 секунд
- **Количество шагов**: 60
- **Скорость обучения**: 0.105 примеров/сек
- **Trainable параметры**: 9,232,384 из 1,552,946,688 (0.59%)
- **Loss на старте**: 1.9284
- **Loss на финише**: 1.0823
- **Снижение loss**: 43.9%

### Конфигурация оборудования
- **Платформа**: Windows
- **Среда**: Python venv (llm_env)
- **Библиотеки**: transformers, peft, torch, datasets

## Выводы

1. **Эффективность LoRA подтверждена**: Метод позволил значительно улучшить качество ответов по специализированной тематике, добавив всего 0.59% новых параметров к модели (9.2 млн из 1.55 млрд).
...
5. **Ресурсная эффективность**: Обучение заняло менее 38 минут на GPU, что демонстрирует эффективность LoRA для быстрой адаптации больших моделей под узкие предметные области.

6. **Стабильность обучения**: График потерь показывает плавное снижение без резких колебаний, что свидетельствует о правильно подобранных гиперпараметрах.
...
**Время обучения:** 37 минут 55 секунд  
**Улучшение loss:** 43.9% (с 1.9284 до 1.0823)
```
